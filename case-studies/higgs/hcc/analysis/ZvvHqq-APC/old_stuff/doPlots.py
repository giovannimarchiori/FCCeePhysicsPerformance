...
#__________________________________________________________
def mapHistos(var, label, sel, param): #var is leptons_p, jets_p... label is ZH, selection is selN
...
#__________________________________________________________
def runPlots(var,param,hsignal,hbackgrounds,extralab,sel):
    ...
    if 'ee' in param.collider:
        ...
        rt = '#sqrt{{s}} = {:.1f} GeV  L = {:.0f} ab^{{-1}},{:10d} sig. {:10d} bkg. '.format(param.energy,intLumiab,int(NSignal),int(Nbackground))
...
def drawStack(name, ylabel, legend, leftText, rightText, formats, directory, logY, stacksig, histos, histosig, histobkg, colors, ana_tex, extralab):
    ...
    #if stacksig:   ##check what is stacksig
    #    hStack.Add(histos[0])
    ...
    for h in iterh:
        h.SetLineWidth(0)
        #if (k == len(histobkg)+len(histosig)-1):
        #    h.SetLineColor(ROOT.kBlack)
        #    h.SetLineWidth(2)
        h.SetFillColor(colors[k])
        hStack.Add(h)
    ...
    for var in param.variables:
        for label, sels in param.selections.items():
            for sel in sels:
                ...
                runPlots(var+'_'+label+'_'+sel,param,hsignal,hbackgrounds,param.extralabel[sel],sel)
